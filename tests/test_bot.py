...
def menu_bot(ctx):
	...
	if ctx.frame == 32:
		...
		# long menu scrolls: last item is visible when selected
		game.menu_index = len(items) - 1
		first = game.menuScroll(len(items))
		y = 228 + (game.menu_index - first) * 20
		ctx.check("last of %d menu items visible when selected (y %d)" % (len(items), y), y + 16 <= 416)
		game.menu_index = 0
